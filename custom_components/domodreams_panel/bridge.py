...
class PanelBridge:
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        # split-segment index: "<tileId>.<segId>" -> (tile, segment).
        ...
        # entities referenced by a DYNAMIC statusDot binding:
    ...
    def _index_config(self) -> None:
        ...
        for page in self.panels.get("pages", []):
            ...
            for tile in page.get("tiles", []) or []:
                ...
                # mirror the entities its state rules reference.
    ...
    def _run_notification_on_press(self, data: dict[str, Any], status: Any) -> None:
        ...
        which button was pressed and HA does the rest (the app owns
    ...
    def _on_event(self, msg: mqtt.ReceiveMessage) -> None:
        ...
        page = data.get("page")
        # Split-segment presses ride the visual state at press time:
        # {button:"<tileId>.<segId>", action:"press", state:"off|on|inset"}.
        ...
        # whether a binding handles it. The state (when present)
    ...
    def _execute_binding(
        ...
        # press:<state>. The bindings live on the segment, not
        # the tile.
    ...
    async def _run_binding(
        ...
        dicts run STRICTLY IN ORDER (multi-action GLOBAL upgrade).
...
def _state_payload(
    ...
    app-side dot comparison works for locks, sensors, anything.
